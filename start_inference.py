...
parser.add_argument('--input_dir_path', required=True, type=str, help="input directory path that contain obj files.")
parser.add_argument('--split_txt_path', required=True, type=str, help="split txt path.")
parser.add_argument('--save_path', type=str, required=True, help="result save directory.")
parser.add_argument('--model_name', type=str, required=True,
                    help="model name. list: tsegnet | tgnet | pointnet | pointnetpp | dgcnn | pointtransformer")
parser.add_argument('--checkpoint_path', required=True, type=str, help="checkpoint path.")
...
pred_obj = ScanSegmentation(
    make_inference_pipeline(args.model_name, [args.checkpoint_path + ".h5", args.checkpoint_path_bdl + ".h5"]))
os.makedirs(args.save_path, exist_ok=True)
print(f"Total are:{len(stl_path_ls)}")
# if len(stl_path_ls) > 20:
#     for i in range(21):
#         print(f"Processing: ", i, ":", stl_path_ls[i])
#         base_name = os.path.basename(stl_path_ls[i]).split(".")[0]
#         pred_obj.process(stl_path_ls[i],
#                          os.path.join(args.save_path, os.path.basename(stl_path_ls[i]).replace(".obj", ".json")))
# else:
